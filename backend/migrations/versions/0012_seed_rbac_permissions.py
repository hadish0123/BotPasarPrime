...
def upgrade() -> None:
    ...
    for name in ROLE_PERMISSIONS:
        ...
        if "description" in role_columns:
            values["description"] = f"System role: {name}"
        if "tenant_id" in role_columns:
    ...
    for role_name, expression in ROLE_PERMISSIONS.items():
        ...
        if expression == "*,-admins.write":
            keys = set(PERMISSIONS) - {"admins.write"}
        for key in keys:
            role_id = existing_roles[role_name]
            permission_id = existing_permissions[key]
            exists = bind.execute(sa.select(role_permission.c.role_id).where(
...
def downgrade() -> None:
    ...
    role = sa.table("roles", sa.column("id", sa.Integer), sa.column("name", sa.String(50)))
    role_permission = sa.table("role_permissions", sa.column("role_id", sa.Integer))
    role_ids = [row.id for row in bind.execute(sa.select(role.c.id).where(role.c.name.in_(list(ROLE_PERMISSIONS)))).all()]
    if role_ids:
