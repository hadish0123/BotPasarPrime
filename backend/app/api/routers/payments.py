...
async def read_payment(payment_id: int, tenant_id: int, claims=Depends(bearer), db: AsyncSession = Depends(get_db)):
    ...
    user_id = claims.get("user_id") or claims.get("sub")
    if not claims.get("is_platform_owner") and (not order or not user_id or order.user_id != int(user_id)): raise HTTPException(403, "forbidden")
...
async def submit_payment(payment_id: int, tenant_id: int, reference: str, claims=Depends(bearer), db: AsyncSession = Depends(get_db)):
    ...
    payment = await get_payment(db=db, tenant_id=tenant_id, payment_id=payment_id)
    if not payment: raise HTTPException(404, "payment_not_found")
    order = await db.get(Order, payment.order_id); user_id = claims.get("user_id") or claims.get("sub")
    if not order or not user_id or order.user_id != int(user_id): raise HTTPException(403, "forbidden")
    if not reference or len(reference.strip()) > 150: raise HTTPException(400, "invalid_reference")
    try:
        transition(payment, "submitted"); payment.reference = reference.strip(); await db.commit()
...
async def verify_payment(payment_id: int, tenant_id: int, approve: bool = True, claims=Depends(require_permission("payments.verify")), db: AsyncSession = Depends(get_db)):
    ...
    if not payment: raise HTTPException(404, "payment_not_found")
    order = await db.get(Order, payment.order_id)
    if not order or order.tenant_id != tenant_id: raise HTTPException(404, "order_not_found")
    try:
        if approve:
    ...
    if approve:
        try:
            ...
            await db.rollback(); raise HTTPException(502, "order_fulfillment_failed") from exc
    return {"id": payment.id, "order_id": payment.order_id, "status": payment.status, "service_id": service.id if service else None}
...
async def expire_payment(payment_id: int, tenant_id: int, claims=Depends(require_permission("payments.verify")), db: AsyncSession = Depends(get_db)):
    ...
    if not payment: raise HTTPException(404, "payment_not_found")
    try:
        transition(payment, "expired"); await db.commit(); return {"id": payment.id, "status": payment.status}
    except ValueError as exc:
        await db.rollback(); raise HTTPException(409, str(exc)) from None
...
async def refund_payment(payment_id: int, tenant_id: int, claims=Depends(require_permission("payments.refund")), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        await db.commit(); return {"id": payment.id, "status": payment.status, "order_id": order.id}
    except ValueError as exc:
        await db.rollback(); raise HTTPException(409, str(exc)) from None
