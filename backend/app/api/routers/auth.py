...
async def tenant_telegram_auth(tenant_id: int, init_data: str | None = None, x_telegram_init_data: str | None = Header(default=None)):
    ...
    async with SessionLocal() as db:
        ...
        membership = await db.scalar(select(TenantUser).where(TenantUser.tenant_id == tenant_id, TenantUser.user_id == user.id, TenantUser.status == "active"))
        if not membership:
            raise HTTPException(403, "tenant access denied")
        permissions = ["auth.telegram", "users.read", "products.read", "orders.read", "orders.write", "payments.write", "services.read", "tickets.write"]
        return {"access_token": create_token(int(user.id), {"telegram_id": telegram_id, "user_id": int(user.id), "tenant_id": tenant_id, "role": membership.role, "permissions": permissions}), "token_type": "bearer", "tenant_id": tenant_id}
