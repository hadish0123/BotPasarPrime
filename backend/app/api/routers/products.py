...
async def products(tenant_id: int, claims=Depends(bearer), db: AsyncSession = Depends(get_db)):
    if "products.read" not in claims.get("permissions", []) and "auth.telegram" not in claims.get("permissions", []):
...
async def product_plans(product_id: int, tenant_id: int, claims=Depends(bearer), db: AsyncSession = Depends(get_db)):
    if "products.read" not in claims.get("permissions", []) and "auth.telegram" not in claims.get("permissions", []):
        raise HTTPException(403, "forbidden")
    _tenant(claims, tenant_id)
    product = await db.scalar(select(Product).where(Product.id == product_id, Product.tenant_id == tenant_id, Product.active.is_(True)))
    if not product:
    ...
    return [{"id": p.id, "name": p.name, "base_price": str(calculate_price(p.price, p.discount_kind or "none", p.discount_value or 0).base_price), "discount": str(calculate_price(p.price, p.discount_kind or "none", p.discount_value or 0).discount), "price": str(calculate_price(p.price, p.discount_kind or "none", p.discount_value or 0).final_price), "duration_days": p.duration_days, "quota_gb": p.quota_gb, "active": p.active} for p in plans]
...
async def create_plan_route(x: PlanCreate, product_id: int, tenant_id: int, claims=Depends(require_permission("products.write")), db: AsyncSession = Depends(get_db)):
    _tenant(claims, tenant_id)
    product = await db.scalar(select(Product).where(Product.id == product_id, Product.tenant_id == tenant_id))
    if not product:
        raise HTTPException(404, "product_not_found")
    try:
        plan = await create_plan(db=db, tenant_id=tenant_id, product_id=product_id, name=x.name, price=x.price, duration_days=x.duration_days, quota_gb=x.quota_gb, discount_kind=getattr(x, "discount_kind", "none"), discount_value=getattr(x, "discount_value", 0))
        await db.commit()
