...
async def register_referral(
    ...
    try:
        referral = await create_referral(
            db, tenant_id, referrer_user_id, current_user, code
        )
        await db.commit()
...
async def referral(
    ...
    current_user = int(claims.get("user_id") or claims.get("sub"))
    if (
        item.inviter_user_id != current_user
        and item.invited_user_id != current_user
        and not claims.get("is_platform_owner")
        and "referrals.read" not in claims.get("permissions", [])
    ):
        raise HTTPException(403, "forbidden")
    return {
...
async def add_commission_ledger(
    ...
    claims=Depends(require_permission("referrals.write")),
    db: AsyncSession = Depends(get_db),
):
    require_tenant_match(tenant_id, claims)
    try:
