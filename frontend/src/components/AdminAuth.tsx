...
function hasAdminAccess(session: Session) {
  return Boolean(
    session.is_platform_owner ||
      (session.tenant_id && (session.permissions || []).some((permission) => permission.startsWith("dashboard."))),
  );
}
    ...
    const authenticate = async () => {
      try {
        ...
        if (existing) {
          ...
          if (active && hasAdminAccess(current)) {
            setSession(current);
            return;
          }
        ...
        if (active) setError(err instanceof Error ? err.message : "احراز هویت مدیریت انجام نشد.");
      } finally {
